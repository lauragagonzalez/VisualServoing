...
def is_corner(front, left, right):
    # condición de bloqueo fuerte delante
    front_blocked = front < 0.5

    # paredes a ambos lados o muy asimétricas
    side_close = (left < 0.4 and right < 0.6) or (right < 0.4 and left < 0.6)

    # diferencia grande (típico de esquina)
    imbalance = abs(left - right) > 0.25

    return front_blocked and (side_close or imbalance)
...
def follow_ball(ball, width):
    ...
    if ball:
        ...
        base = 2.8 + 2.0 * np.tanh(2.0 * dist_error)
        base = np.clip(base, 0.0, 5.5)

        turn = 1.2 * error
        turn = np.clip(turn, -1.5, 1.5)

        slow_factor = np.clip(1 - 0.3 * abs(error), 0.5, 1.0)
        base *= slow_factor
...
def controller(sonar, ball, width):
    ...
    if corner_mode:
        if front > 0.9:
            ...
            corner_cmd = (-0.2, 2.0) if corner_dir == 1 else (2.0, -0.2)
            if ball is not None:
